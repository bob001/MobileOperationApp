...
//    public static final int VERSION = 2;//在版本1的基础上，在数据库表中增加了lon和lat经纬度字段
    public static final int VERSION = 3;//在版本2的基础上，在数据库表中增加了task_code任务编码字段
    ...
    public void saveRepairInfo(RepairInfo ri) {
        if (ri != null) {
            ...
            values.put("task_code", ri.getTaskCode());
            ...
            values.put("lon", ri.getLon());
            values.put("lat", ri.getLat());
    ...
    public List<RepairInfo> loadRepairInfos() {
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                ri.setTaskCode(cursor.getString(cursor.getColumnIndex("task_code")));
                ...
                ri.setLon(cursor.getDouble(cursor.getColumnIndex("lon")));
                ri.setLat(cursor.getDouble(cursor.getColumnIndex("lat")));
