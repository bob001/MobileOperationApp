...
//                Toast.makeText(getContext(), "工具提醒", Toast.LENGTH_SHORT).show();
            ...
            case R.id.tv_comehere:
                Intent intentComeHere = new Intent(MyApplication.getContext(),MapMainActivity.class);
                intentComeHere.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                intentComeHere.putExtra("lon",repairInfo.getLon());
                intentComeHere.putExtra("lat",repairInfo.getLat());
                MyApplication.getContext().startActivity(intentComeHere);
